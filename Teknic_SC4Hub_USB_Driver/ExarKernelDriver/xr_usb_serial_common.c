 ...
 *	XR22801/802/804
 *
 * The driver has been tested on various kernel versions from 3.6.x to 6.18.x.
 ...
 *            Version 1H - Fixes for Kernel 6.18.
 */
...
static ssize_t show_country_codes(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev);
	struct xr_usb_serial *xr_usb_serial = usb_get_intfdata(intf);
...
static ssize_t show_country_rel_date(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev);
	struct xr_usb_serial *xr_usb_serial = usb_get_intfdata(intf);
...
static int get_serial_info(struct xr_usb_serial *xr_usb_serial, struct serial_struct __user *info)
{
	...
	tmp.closing_wait = xr_usb_serial->port.closing_wait == ASYNC_CLOSING_WAIT_NONE ? ASYNC_CLOSING_WAIT_NONE : xr_usb_serial->port.closing_wait / 10;
...
skip_countries:
	...
	rv = gpiochip_add_data(&xr_usb_serial->xr_gpio, xr_usb_serial);
#else
	gpiochip_base = 0;
...
static void xr_usb_serial_disconnect(struct usb_interface *intf)
{
	...
	if (!xr_usb_serial->combined_interfaces)
		usb_driver_release_interface(&xr_usb_serial_driver, intf == xr_usb_serial->control ? xr_usb_serial->data : xr_usb_serial->control);
