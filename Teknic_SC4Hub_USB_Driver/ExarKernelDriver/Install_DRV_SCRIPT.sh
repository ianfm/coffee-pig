...
if [[ ! -d "/lib/modules/$(uname -r)/build" ]]; then
    echo "Kernel headers not found at /lib/modules/$(uname -r)/build"
    echo "Please install kernel headers and build tools, then re-run."
    exit 1
fi
...
modprobe -r xr_usb_serial_common >/dev/null 2>&1 || true

# Run the driver makefile
make

if [[ ! -f "$SCRIPT_DIR/xr_usb_serial_common.ko" ]]; then
    echo "Build failed: xr_usb_serial_common.ko not found."
    exit 1
fi

# Copy diver module into modules
cp "$SCRIPT_DIR/xr_usb_serial_common.ko" /lib/modules/"$(uname -r)"/kernel/drivers/usb/serial

# Refresh modules
depmod -a

# make module load at boot time by adding "xr_usb_serial_common" on its own line to /etc/modules
if ! grep -qxF "xr_usb_serial_common" /etc/modules; then
    echo "xr_usb_serial_common" >> /etc/modules
fi
